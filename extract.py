...
from model_utils import get_wrapper
...
def main():
    ...
    wrapper = get_wrapper(model_name=args.model, device=args.device)

    storage = {"attns": [], "acts": [], "logits": []}
    ...
    for idx, batch in enumerate(tqdm(loader, desc="Extracting", unit="batch")):
        ...
        attn_batch = np.stack([t.detach().cpu().numpy() for t in out["attns"]], axis=0)
        act_batch = np.stack([h.detach().cpu().numpy() for h in out["acts"]], axis=0)

        storage["attns"].append(attn_batch)
        storage["acts"].append(act_batch)
    ...
    )  # [num_layers, total_samples, heads, seq, seq]
    acts = np.concatenate(
        storage["acts"], axis=1
    )  # [num_layers+1, total_samples, seq, hidden_dim]
    logits = np.concatenate(storage["logits"], axis=0)  # [total_samples, seq, vocab]
